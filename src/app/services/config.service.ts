 ...
 * Runs as an app initializer, so assets/config.json is guaranteed to be loaded
 * before AppComponent's constructor runs `oauthStrategy.setOptions(...)`.
 *
 * (provideAppInitializer is the Angular 19+ replacement for the deprecated
 * APP_INITIALIZER multi-provider; the callback runs in an injection context,
 * which is why inject() works here.)
 */
export const CONFIG_PROVIDERS: EnvironmentProviders[] = [
  provideAppInitializer(() => inject(ConfigService).init('./assets/config.json')),
];
